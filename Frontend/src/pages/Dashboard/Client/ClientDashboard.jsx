...
function ClientDashboard() {
  ...
  // State for cases and loading status
  ...
  useEffect(() => {
    const fetchUserCases = async () => {
      ...
      try {
        ...
        console.log("User cases:", res.data);
        setCases(Array.isArray(res.data) ? res.data : []);
      } catch (error) {
        console.error("Error fetching user cases:", error.response?.data || error.message);
        if (error.response?.status === 404) {
          setCases([]); // No cases or endpoint not found, handle silently
  ...
  // Get the first caseId if available
